# DMXAPI MiniMax-Hailuo-02 首尾帧生成视频 API 使用文档

MiniMax-Hailuo-02 首尾帧生成视频 API 支持上传首帧和尾帧两张图片，结合文本描述自动生成平滑过渡的视频。模型支持 15 种运镜指令（如左右摇、推拉、升降、变焦等），可通过 `[指令]` 语法实现精确的镜头控制，支持组合运镜与顺序运镜。输出视频支持 768P 和 1080P 两种分辨率，最长可生成 10 秒视频。
...
| 模型名称 | 说明 |
|---------|------|
| `MiniMax-Hailuo-02` | 首尾帧生成视频，提交任务 |
| `MiniMax-Hailuo-query` | 查询视频生成任务状态 |
| `MiniMax-Hailuo-get` | 获取生成视频的下载链接 |
...
payload = {
    ...
    # 顺序运镜: prompt 中前后出现的指令依次生效
    "input": "A little girl grow up.",

    # 【first_frame_image】(string, 可选) 将指定图片作为视频的起始帧
    # 支持公网 URL 或 Base64 编码的 Data URL (data:image/jpeg;base64,...)
    # 图片要求: 格式 JPG/JPEG/PNG/WebP，体积小于 20MB
...
payload = {
    # 【model】(string, 必填) 查询任务状态的模型标识
    # 固定值: "MiniMax-Hailuo-query"
...
payload = {
    # 【model】(string, 必填) 下载视频的模型标识
    # 固定值: "MiniMax-Hailuo-get"
...
1. **提交任务** → 调用 `POST /v1/responses`（model: `MiniMax-Hailuo-02`），上传首尾帧图片和文本描述，获得 `task_id`
...
<p align="center">
  <small>&copy; 2026 DMXAPI MiniMax-Hailuo-02 首尾帧生成视频</small>
</p>
