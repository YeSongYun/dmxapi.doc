...
基于字节跳动 Seedance 2.0 多模态视频生成模型，doubao-seedance-2-0-260128 支持参考图片和视频进行智能视频编辑，可实现场景替换、主体替换、视频延长、风格迁移等多种编辑效果。支持多模态输入（文本 + 图片 + 视频 + 音频组合），最多接受 9 张参考图和 3 段参考视频，生成分辨率支持 480p/720p，时长 4~15 秒可控。采用异步任务模式，提交任务后获取任务 ID，再通过专用接口获取生成的视频 URL。
