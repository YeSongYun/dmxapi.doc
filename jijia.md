...
- 首次将内容写入缓存时触发，按 **缓存写入 tokens** 数单独计费
- 费率因模型而异，请参照 [模型价格](https://www.dmxapi.cn/rmb) 中对应模型的缓存写入单价

### 缓存命中（Cache Read）

- 后续请求命中已有缓存时触发，按 **缓存命中 tokens** 数单独计费
- 费率因模型而异，通常低于普通输入价格，请参照 [模型价格](https://www.dmxapi.cn/rmb) 中对应模型的缓存读取单价
