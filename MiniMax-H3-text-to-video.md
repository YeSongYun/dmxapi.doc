...
MiniMax-H3 视频生成 V2 接口，通过多模态 `input` 数组输入（文本 / 图片 / 视频 / 音频）驱动，2K 直出。文生视频场景仅需在数组中提供一个非空文本项，单个 `text` 最多 7000 个字符；支持 768P 与 2K 两档分辨率、4~15 秒整数时长，以及 21:9、16:9、4:3、1:1、3:4、9:16 六种宽高比（文生视频场景下 `ratio` 必填且不可为 `adaptive`）。接口以异步任务方式工作：提交后返回任务 ID，再用查询模型换取最终视频地址。
