# DMXAPI MiniMax-Hailuo-02 图生视频 API 使用文档

MiniMax-Hailuo-02 是 MiniMax 海螺系列的图生视频模型，支持将指定图片作为起始帧并结合文本描述生成高质量视频。支持 512P/768P/1080P 多种分辨率输出，提供 15 种运镜指令实现精确镜头控制，支持 6 秒和 10 秒两种时长，并可通过 prompt 优化器自动增强描述效果。
...
| 模型名称 | 说明 |
|---------|------|
| `MiniMax-Hailuo-02` | 图生视频，支持 512P/768P/1080P 分辨率 |
...
payload = {
    # 【model】(string, 必填) 查询任务使用固定模型名 "MiniMax-Hailuo-query"
...
payload = {
    # 【model】(string, 必填) 下载视频使用固定模型名 "MiniMax-Hailuo-get"
...
1. **提交任务** → 调用 `/v1/responses`（model: `MiniMax-Hailuo-02`），传入图片和文本描述，获得 `task_id`
2. **轮询查询** → 定期调用 `/v1/responses`（model: `MiniMax-Hailuo-query`），传入 `task_id`，等待 `status: "Success"`，获得 `file_id`
3. **下载视频** → 调用 `/v1/responses`（model: `MiniMax-Hailuo-get`），传入 `file_id`，获取 `download_url` 并下载视频文件

<p align="center">
  <small>© 2026 DMXAPI MiniMax-Hailuo-02 图生视频</small>
</p>
