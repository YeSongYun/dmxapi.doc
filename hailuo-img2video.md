...
- **`MiniMax-Hailuo-2.3-Fast`** - 快速版本，生成速度更快
- **`MiniMax-Hailuo-02`** - 基础版本，支持 512P 分辨率
...
从 `/v1/files/retrieve` 接口获取的 `download_url` 包含签名参数，具有时效性。建议获取后及时下载或保存视频文件。

### 运镜指令使用建议

- 单个场景建议使用 1-2 个运镜指令
- 组合运镜时建议不超过 3 个指令
- 使用标准指令（如 `[推进]`、`[拉远]`）比自然语言描述更准确
