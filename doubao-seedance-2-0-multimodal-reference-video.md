...
基于豆包 Seedance 2.0 模型的多模态参考生视频接口，支持同时输入参考图片（1~9 张）、参考视频（1~3 个）、参考音频（1~3 段）及文本提示词，生成高度贴合参考素材的目标视频。支持生成全新视频、编辑视频和延长视频三种创作模式，可生成时长 4~15 秒、最高 1080p 分辨率的有声视频。采用异步任务模式，提交后通过独立查询接口获取结果。
