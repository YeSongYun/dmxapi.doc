# wan2.7-i2v-2026-04-25 首尾帧生视频 API 使用文档

`wan2.7-i2v-2026-04-25` 是通义万相 2.7 系列的图生视频模型，通过 `/v1/responses` 端点以「提交任务 + 轮询查询」两步异步方式调用。本文档演示首尾帧生视频用法：同时传入首帧与尾帧两张图像，模型自动补全两帧之间的运动过渡，生成最长 15 秒、最高 1080P 的有声视频，还可再叠加一段 2~30 秒的驱动音频（first_frame+last_frame+driving_audio 组合）。图像支持 JPEG/JPG/PNG/BMP/WEBP 格式、宽高 [240, 8000] 像素、公网 URL 或 Base64 编码传入，输出视频宽高比自动跟随首帧图像，适合镜头运镜设计、转场动画、首尾呼应的创意短片等场景。
    ...
    "input": {
        ...
        "media": [
            ...
            },
            {
                ...
                "type": "last_frame",
                # 【media[].url】(string, 必填) 尾帧图像 URL 或 Base64 编码数据
                # 图像限制与首帧相同: 格式 JPEG/JPG/PNG（不支持透明通道）/BMP/WEBP；
                # 宽高 [240, 8000] 像素；宽高比 1:8 ~ 8:1；文件大小不超过 20MB
...
```json
{
  "request_id": "8fc6cb0d-e382-9122-9224-6353aaa6d0b0",
  "output": [
    {
      "type": "message",
      "content": [
        {
          "type": "output_text",
          "text": "{\"task_id\":\"065a2cbf-61a9-4721-b9e2-2f3016fbb09c\",\"task_status\":\"PENDING\"}"
        }
      ]
    }
  ],
...
payload = {
    ...
    "model": "wan2.7-get",
    "input": "065a2cbf-61a9-4721-b9e2-2f3016fbb09c"
...
```json
{
  "request_id": "a6c065ae-6603-93a1-ba75-0bd715a56198",
  "output": [
    {
      "type": "message",
      "content": [
        {
          "type": "output_text",
          "text": "{\"task_id\":\"065a2cbf-61a9-4721-b9e2-2f3016fbb09c\",\"task_status\":\"SUCCEEDED\",\"submit_time\":\"2026-07-14 16:48:15.438\",\"scheduled_time\":\"2026-07-14 16:48:15.484\",\"end_time\":\"2026-07-14 16:48:36.998\",\"orig_prompt\":\"写实风格,一只小黑猫好奇地仰望天空,镜头从平视角度逐渐升高,最后以俯视角度捕捉到它好奇的眼神。\",\"video_url\":\"https://dashscope-a717.oss-accelerate.aliyuncs.com/1d/41/20260714/fa14b052/26824650-metadata_user_0e30fad2e464209e_watermark.mp4?Expires=1784105315&OSSAccessKeyId=LTAI5tJjG6wsHad1Sf7iezX4&Signature=dcV5cwe7BxF036Se63CoPltQTiw%3D\"}"
        }
      ]
    }
  ],
...
<p align="center">
  <small>© 2026 DMXAPI wan2.7-i2v-2026-04-25 首尾帧生视频</small>
</p>
