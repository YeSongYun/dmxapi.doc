...
| 模型 | 适用范围 |
|------|----------|
| `paiwo-video` | paiwo-itv-hd 数字人 lipsync |
| `pixverse-video` | PixVerse-V6 视频延长 |

::: warning 选对上传模型（用错会报 701009）
两个上传模型的 `media_id` **互不通用**，请按下表选用与功能配套的上传模型；用错会报 `701009 (You can only query your own generated or uploaded content)`。

| 你要用的功能 | 用这个上传模型 |
|---|---|
| paiwo-itv-hd 数字人 lipsync | `paiwo-video` |
| PixVerse-V6 视频延长 | `pixverse-video` |
:::
...
## pixverse-video 示例（PixVerse-V6 视频延长）
...
payload = {
    # 【model】(string, 必需) 模型名称，固定值为 pixverse-video
    # 用于 PixVerse-V6 视频延长，上传成功后返回的 media_id 作为 video_media_id 使用
...
payload = {
    # 【model】(string, 必填) 指定上传资源所用模型
    # 使用 "pixverse-video" 上传，供 PixVerse-V6 视频延长引用（返回的 media_id 作为 video_media_id）
    "model": "pixverse-video",

    # 【input】(string, 必填) 待上传视频的 URL 地址
    # 支持视频格式: mp4 / mov / webm / quicktime
    # 视频限制: 最大分辨率 1920，最大文件大小 50MB，最大时长 30 秒
    "input": "https://www.runoob.com/try/demo_source/mov_bbb.mp4",  # 替换为实际视频 URL
}
