# 豆包 doubao-seedream-5.0-lite 多图融合 API 使用文档

基于字节跳动豆包最新 Seedream 5.0 Lite 图像创作模型的多图融合接口，支持普通请求与流式输出两种调用方式。通过输入多张参考图片（2-14 张）结合文本提示词，将多张图片的元素、风格、构图等信息智能融合生成新图片。可结合组图模式批量生成最多 15 张融合结果（输入参考图数量 + 生成图片数量 ≤ 15），内置联网检索和提示词优化功能，输出 PNG、JPEG 格式，支持 2K/4K 分辨率及多种宽高比。
...
payload = {
    ...
    # 【input】(string, 必填) 多图融合提示词
    # 用于描述想要如何融合多张参考图片的内容，支持中英文输入
    ...
    # 【image】(array, 必填) 输入的多张参考图片信息
    # 多图融合场景下需传入 2-14 张参考图片，支持 URL 或 Base64 编码
    # 图片要求:
    #   格式: jpeg, png, webp, bmp, tiff, gif
    #   宽高比范围: [1/16, 16]
    #   宽高长度(px) > 14
    #   单张大小: 不超过 10MB
    #   单张总像素: 不超过 6000x6000=36000000 px
...
payload = {
    ...
    # 【image】(array, 必填) 输入的多张参考图片信息
    # 多图融合需传入 2-14 张参考图片，支持 URL 或 Base64 编码
    # 图片要求: 格式 jpeg/png/webp/bmp/tiff/gif，单张 <= 10MB，总像素 <= 36000000 px
    # 注意: 输入参考图数量 + 生成图片数量 <= 15 张
    "image": [
        "https://ark-project.tos-cn-beijing.volces.com/doc_image/seedream4_imagesToimage_1.png",
        "https://ark-project.tos-cn-beijing.volces.com/doc_image/seedream4_5_imagesToimage_2.png"
    ],
...
data: {"content_index":0,"delta":"![Image 1](https://...图片URL...)","item_id":"msg_614c7af476a9410d994da160c105040f","logprobs":[],"output_index":0,"sequence_number":4,"type":"response.output_text.delta"}
event: response.output_text.delta
data: {"content_index":0,"delta":"![Image 2](https://...图片URL...)","item_id":"msg_614c7af476a9410d994da160c105040f","logprobs":[],"output_index":0,"sequence_number":5,"type":"response.output_text.delta"}
event: response.output_text.delta
data: {"content_index":0,"delta":"![Image 3](https://...图片URL...)","item_id":"msg_614c7af476a9410d994da160c105040f","logprobs":[],"output_index":0,"sequence_number":6,"type":"response.output_text.delta"}
...
<p align="center">
  <small>© 2026 DMXAPI 豆包 doubao-seedream-5.0-lite 多图融合</small>
</p>
