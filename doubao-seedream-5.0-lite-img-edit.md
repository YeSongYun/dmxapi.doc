# doubao-seedream-5.0-lite 图片编辑 API 使用文档

基于字节跳动 Seedream 5.0 lite 模型的图片编辑接口，通过 DMXAPI `/v1/responses` 端点调用。输入一张参考图 + 文本指令即可完成局部修改、风格迁移、材质替换等智能编辑，支持 2K / 3K / 4K 三档分辨率、组图批量生成、联网搜索增强（`tools: web_search`）、自定义输出格式（`png` / `jpeg`）与流式输出。参考图支持 URL 或 Base64 编码，单图最大 30MB，适合电商修图、创意设计、批量风格化等场景。

## 接口地址

| 接口 | 请求方式 | URL |
|------|---------|-----|
| 图片编辑 | POST | `https://www.dmxapi.cn/v1/responses` |
...
payload = {
    ...
    # 图片编辑场景必填；doubao-seedream-5.0-lite 支持单图或多图输入，最多传入 14 张参考图
...
<p align="center">
  <small>© 2026 DMXAPI doubao-seedream-5.0-lite 图片编辑</small>
</p>
